import React from 'react'

function BecomeVolunteer() {
    return (
        <div className="volunteer">
        <h1 style={{textAlign:'center' ,color:'#000'}}>Become A Volunteer</h1>
           
            <p style={{textAlign:'center',color:'#000'}}>So seed seed green that winged cattle in. Gathering thing made 
            fly you're divided deep leave on the medicene moved us land years living.
            <p className="button"><a href="https://docs.google.com/forms/d/e/1FAIpQLScElLmk-MuBzgNdWiY2LCdJ8W_kGeaseegRp4xT_GGGFRtylQ/viewform?usp=pp_url">JOIN US TODAY</a></p>
            </p>
           
        </div>
    )
}

export default BecomeVolunteer
